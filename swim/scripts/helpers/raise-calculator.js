export async function raise_calculator() {
    new Dialog({
        title: game.i18n.localize("SWIM.raise-calculator.label"),
        content: `
            <form>
                <div class="form-group">
					<div style="display: flex; align-items: center;">
						<img style="margin-right: 10px; border: 0;" src="modules/swim/assets/icons/misc/bullseye.svg" alt="" width="25" height="25" />
						<label><b>${game.i18n.localize("SWIM.raise-calculator.target-number")}:</b></label>
					</div>
                ...
                <div class="form-group">
					<div style="display: flex; align-items: center;">
						...
						<label><b>${game.i18n.localize("SWIM.raise-calculator.roll")}:</b></label>
					</div>
                ...
                <div style="display: flex; align-items: center;">
					...
					<p name="calculation"><b>${game.i18n.localize("SWIM.raise-calculator.result")}</b></p>
				</div>
            </form>`,
        buttons: {},
                ...
                ["input", "change"].forEach((evt) => {
                    dialogContent.querySelector(`input[name="${input}"]`).addEventListener(evt, (event) => {
                        ...
                        if (parseInt(target) > parseInt(result)) {
                            ...
                            calcResult.innerHTML = `<b>${game.i18n.localize("SWIM.raise-calculator.failure")}</b>`;
                        } else if (parseInt(target) <= parseInt(result) && raises < 1) {
                            calcImage.src = "modules/swim/assets/icons/misc/raise_yellow.svg";
                            calcResult.innerHTML = `<b>${game.i18n.localize("SWIM.raise-calculator.success")}</b>`;
                        } else {
                            calcImage.src = "modules/swim/assets/icons/misc/raise_green.svg";
                            if (raises < 2) {
                                calcResult.innerHTML = `<b>${game.i18n.format("SWIM.raise-calculator.raises.singular", {
                                    raises,
                                })}</b>`;
                            } else {
                                calcResult.innerHTML = `<b>${game.i18n.format("SWIM.raise-calculator.raises.plural", {
                                    raises,
                                })}</b>`;
                            }
                        }
