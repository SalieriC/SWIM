...
export async function brsw_hooks() {
    ...
    Hooks.on('BRSW-RollItem', (card, _) => {
        ...
        if (backlashSetting === 'disabled' || !game.settings.get("swim", "br2Support")) { return }
...
async function update_brsw_injury(effect, type) {
    console.log("SWIM | You are using SWIM with BRSW support, injury Active Effects from BRSW will be adjusted. Please report on the SWIM repository in case of unexpected results and bugs!")
