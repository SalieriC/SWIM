...
export function swim_buttons(hudButtons) {
    ...
    let tilesButton = hudButtons.find((val) => {
        return val.name == "tiles";
    });
    let chaseButtonIndex = tilesButton.tools.findIndex((object) => {
        return object.name === "clear-chase-cards";
    });
    ...
    let lightHud = hudButtons.find((val) => {
        return val.name == "lighting";
    });
    if (lightHud) {
        if (game.user.isGM) {
            lightHud.tools.push({
